"""Frozen migration 0003: never edit applied schemas."""
...
class Document(Base):
    ...
    current_sha256: Mapped[str | None] = mapped_column(ForeignKey("document_blobs.sha256"))
...
Table("opportunities", Base.metadata, Column("id", String(36), primary_key=True))
...
def view_statements(dialect):
    ...
    yield """CREATE VIEW v_document_coverage AS
        ...
        SUM(CASE WHEN b.extraction_status IS NOT NULL AND b.extraction_status <> 'extracted' THEN 1 ELSE 0 END) AS text_issue_count,
        ...
        SUM(CASE WHEN od.is_root=1 AND d.state='fetched' AND b.extraction_status='extracted' THEN 1 ELSE 0 END) AS roots_fetched,
        ...
        LEFT JOIN document_blobs b ON d.current_sha256=b.sha256 WHERE od.active=1
